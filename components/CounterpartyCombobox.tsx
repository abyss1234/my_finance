...
export default function CounterpartyCombobox({ id, name, options, value, onChange }: Props) {
  const menuId = useId();
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label="Show saved people and shops"
        aria-expanded={isOpen}
        aria-controls={menuId}
      ...
      {isOpen && (
        <div
          id={menuId}
          role="listbox"
          aria-label="Saved people and shops"
        ...
        >
          ...
          {filteredOptions.length === 0 && (
            <p className="px-2 py-3 text-sm text-zinc-500">No matching saved names.</p>
          )}
        </div>
      )}
    </div>
